...
import type { Skill, CategorySkill } from "$lib/server/db/schema_skill";
...
export type CreateSkills = Omit<Skill, 'id' | 'updatedAt'> & {
	iconIds?: string[];
};

export type UpdateSkills = Omit<Skill, 'id' | 'createdAt'> & {
	iconIds?: string[];
};
...
export type CreateCategorySkills = Omit<CategorySkill, 'id' | 'updatedAt'>;
export type UpdateCategorySkills = Omit<CategorySkill, 'id' | 'createdAt'>;
